...
function fnUsage(){
    $.ajax({
        ...
        success:function (result) {
            ...
            $("#totalCPU").text((data.cpu.total).toFixed(2));
...
function fnShowContainsetting(){
    $("#Containsetting").modal("show");
}
function fnGetResourceUsed(){
    $.ajax({
        type: 'GET',
        url: _URL_INTERFACE+'dashboard/app/usage',
        headers: {
            "token": token
        },
        dataType: 'json',
...
function fnGetCpuData(){
    $.ajax({
        type: 'GET',
        url: _URL_INTERFACE+'dashboard/tenant/app/usage',
        headers: {
            "token": token
        },
        dataType: 'json',
...
function fnHandleBasefig(result){
    ...
    for(var i = 0; i < result.length; i++){
        if(result[i].status=='1'){
            getData.push(result[i].sys_name);
        }
    }
    var json = {};
    ...
    $.ajax({
        ...
        success:function (result) {
            ...
            if(data.length==0){
                ...
                $("#eventlist").css('text-align','center');
...
function fnSysStatus(){
    $.ajax({
        type: 'get',
        url: _URL_INTERFACE+'dashboard/sys/status',
        headers: {
            "token": token
        },
        dataType: 'json',
        success:function (result) {
            ...
            if(data && data.length!=0){
                ...
                $("#sya_status").html("<ul id='dataBoxFontCSS'>暂无应用数据</ul>");
            }
        },error:function (XMLHttpRequest, textStatus, errorThrown) {
            if(XMLHttpRequest.status === 401){
                window.location.href = '/index.html';
    ...
    $.ajax({
        ...
        success:function (result) {
            ...
            if(data.length==0){
                ...
                $("#alarminfo").text("暂无告警信息");
                $("#alarminfo").css('text-align','center');
