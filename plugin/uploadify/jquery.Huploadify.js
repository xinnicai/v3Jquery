	  ...
	  var fileObj = {
			  ...
			  if(typeArray.length>0){
				  for(var i=0,len=files.length;i<len;i++){
				  	var thisFile = files[i];
						...
						if($.inArray(thisFile.name.split('.').pop(),typeArray)>=0 || typeArray[0] === '*'){
							arr.push(thisFile);	
						}
						else{
		  ...
		  onSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					var file = files[i];
					//处理模板中使用的变量
					var $html = $(option.itemTemplate.replace(/\${fileID}/g,'fileupload_'+instanceNumber+'_'+file.index).replace(/\${fileName}/g,'文件名：'+file.name).replace(/\${fileSize}/g,formatFileSize(file.size)).replace(/\${instanceID}/g,_this.attr('id')));
					//如果是自动上传，去掉上传按钮
					if(option.auto){
					...
					if(option.showUploadedSize){
						var num = '<span class="progressnum"><span class="uploadedsize">0KB</span>/<span class="totalsize">${fileSize}</span></span><br>'.replace(/\${fileSize}/g,formatFileSize(file.size));
		  ...
		  funUploadFile: function(file) {
			  ...
			  if (xhr.upload) {
				  ...
				  xhr.setRequestHeader("token", token);//自定义token
				  var fd = new FormData();
