var cmpValue = JSON.parse(getUrlParam('container'));
//$('#ResourcesPool').text(cmpValue.resources_production_domain.name);
$('#sysId').text(cmpValue.domain.business_sysdomain.name);
$('#app_name').text(cmpValue.appName);
$('#con_configure').text(cmpValue.container_spec.cpu + "核CPU，" + cmpValue.container_spec.mem + "MB内存，" + cmpValue.container_spec.disk + "MB存储");
$('#maxInstances').text("最大可使用实例数为：" + cmpValue.maxNum);
...
// console.log(cmpValue.orderId);
function getxlsxUrl() {
  $.ajax({
    type: 'get',
    url: "http://20.26.28.187:6062/v3.0/businesscenter/order/containerInitial?id=" + cmpValue.orderId,
    dataType: 'json',
    // data: {id:cmpValue.orderId},
    success: function (result) {
      var fileUrl='';
      if (result.entity.AttachList) {
        for (var i = 0; i < result.entity.AttachList.length; i++) {
          fileUrl += '<a href="http://devopstest.yw.zj.chinamobile.com:10005/v1/businesscenter/file/view/' + result.entity.AttachList[i].id + '" style="text-decoration: underline;">' + result.entity.AttachList[i].name + '</a> ，';
          if(i < result.entity.AttachList.length-1){
            fileUrl += "，";
          }
        }
        $('#getDownloadFile').append(fileUrl);
      }
    },
    error: function () {
      console.log('--------------------------error');
...
function postSuccessToCMP(aaa) {
  // var aaa = "20.26.33.35:31082";
//  $.ajax({
//    type: 'get',
//    url: _URL_INTERFACE + "apps/container/list?app_id=" + jsUuid,
//    dataType: 'json',
//    // data: {id:cmpValue.orderId},
//    success: function (result) {
//      if (result.data) {
//        for (var i = 0; i < result.data.length; i++) {
//          for (var j = 0; j < result.data[i].containers.length; j++) {
//            var bbb = result.data[i].containers[j].host + ':' + result.data[i].containers[j].posts[0];
//            aaa.push(bbb);
//          }
//        }
//      }
...
//  });
  $.ajax({
    type: 'post',
    url: "http://20.26.28.187:6062/v3.0/businesscenter/order/updatestatus",
    dataType: 'json',
    data: JSON.stringify({appid: jsUuid, url: aaa,id:cmpValue.orderId}),
    success: function (result) {
      console.log(result);
    },
    error: function () {
...
function fnCreateData(oldData) {
  ...
  resources_production_domain = {
    'code': cmpValue.resources_production_domain.code,
    'name': cmpValue.resources_production_domain.name
  };
  ...
  var domain = {
    "business_domain": {
      'code': cmpValue.domain.business_domain.code,
      'name': cmpValue.domain.business_domain.name
    },
    "business_subdomain": {
      'code': cmpValue.domain.business_subdomain.name,
      'name': cmpValue.domain.business_subdomain.name
    },
    "business_sysdomain": {
      'code': cmpValue.domain.business_sysdomain.code,
      'name': cmpValue.domain.business_sysdomain.name
    }
  };

  var name = cmpValue.appName;
  ...
  var container_spec = {
    "cpu": cmpValue.container_spec.cpu,
    "mem": cmpValue.container_spec.mem,
    "disk": cmpValue.container_spec.disk
  };
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      result.data.forEach(function (item,index) {
        haUrl.push(item.haproxy_url);
      });
      fnCreateAppLog();
      postSuccessToCMP(haUrl);
    },
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      $("#_NextBtn").text('确定');
      $("#_prevBtn").text('取消');
      $("#_prevBtn").attr('disabled', false);
...
function fnCreateInit() {
  ...
  $("#instances").val('1');
  $("#instances").attr('disabled', false); //实例数
...
function fnGetCluster(code){
  $('#dataCenterTable').bootstrapTable({
    url:  _URL_INTERFACE+"cluster/all?resource_pool="+cmpValue.resources_production_domain.code, method: 'get', cache: false,
    ajaxOptions:{headers: {
      "token": token
    }},async:false,
  ...
  for (var i = 0; i < inputVal.length; i++) {
    ...
    if (aaaa.val() == '') {
      ...
      if (aaaa.attr('id') != 'undefined' && appID == '') {
        ...
        if (aaaa[0].id == "instances" && aaaa.val() > cmpValue.maxNum) {
          aaaa.nextAll(".red").html("最大实例数不能超过" + cmpValue.maxNum + "，请重输");
          return;
        } else {
...
function fnupload(obj, filetype) {
  $(obj).Huploadify({
    auto: true,
    fileTypeExts: '*.*',
    multi: true,
    ...
    onUploadSuccess: function (file, data, response) {
      ...
      if (obj == '#uploadbtn') {
        ...
        editor1 += 'ADD ' + info.raw_filename + ' /app/bin/startServer.sh\n';
        $("#img_remark").val(editor1);
      } else {
        editor1 += 'ADD ' + info.raw_filename + ' /app/pkg/\n';
        $("#img_remark").val(editor1);
      }
    },
...
var timeout = false, img_name_new = '';
...
function setTime(name) {
  ...
  cleanApp = setTimeout(function () {
    setTime(name)
  }, 5000); //time是指本身,延时递归调用自己,100为间隔调用时间,单位毫秒
}
