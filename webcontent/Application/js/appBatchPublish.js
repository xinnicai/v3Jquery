...
// var appIds=JSON.parse(getUrlParam('appIds'));

//要发布的应用展示
function showAppItem(){
  var eleAppitem=""  
  for(var index=0;index<appIds.length;index++){
    eleAppitem +='<div><span>'+appIds[index]+'</span><span class="icon-close" onclick=delAppItem('+index+')>×</span></div>';
  }
  $("#app-items").empty();
  $("#app-items").append(eleAppitem);
}

//删除不需要发布的应用
function delAppItem(index){
  appIds.splice(index,1);
  $("#app-items div").eq(index).remove();
  showAppItem();
}
...
function fnHandleData(data){
  ...
  if(cluster.length){
    ...
    if(clusterLength>1){
      ...
      $("[name='reset_cluster']").append('<p value="'+cluster[0].label+'">当前集群为'+cluster[0].name+'</p>');
    }
  ...
  fnGetVersion(app_origintype);  //获取版本列表信息
...
function fnGetVersion(app_origin){
  ...
  $('#_ReleaseTable').bootstrapTable({
    url:  _URL_INTERFACE+"apps/version?app_id="+app_id, method: 'get', cache: false,
    ajaxOptions:{headers: {
      "token": token
    }},
   dataType: 'json',
