$(document).ready(function (){
  ...
  fnGetPool();
  showSysName();
 /* fnnoUiSlider("#sample-onehandle4","#con_CPU",0,4);
  fnnoUiSlider("#sample-onehandle5","#con_mem",0,8);*/
   // initResourceName();
});
function fnInit(){
  ...
  var cpu_mem=$(".con_configureX").find('.active div').text().split('/');
    ...
    headers: {
      "token": token
       ...
       "innodb_buffer_pool_size":$("#innodb_buffer_pool_size").val(),
      "innodb_log_file_size":$("#innodb_log_file_size").val(),
      "innodb_log_buffer_sizele_size":$("#innodb_log_buffer_sizele_size").val(),
      "max_connections":$("#max_connections").val(),
      "lower_case_table_names":$("#lower_case_table_names").val(),
      "max_connect_errors":$("#max_connect_errors").val(),
      "expire_logs_days":$("#expire_logs_days").val(),
		    "disk":parseFloat($("#sys_disk").val()),
...
function fnHerf(){
  window.location.href = '#/webcontent/server/mysql1.html?seq_id='+$("#sever_id").text()+'&seq_type=mysql&seq_status=1';
}
//获取网络、存储
function fngetData(){
	$.ajax({
